...
PYTHON38_SLIM_TAG = $(REGISTRY):python38-slim
...
# All image tags
ALL_TAGS = $(NOETIC_TAG) $(HUMBLE_TAG) $(FOCAL_TAG) $(FOCAL_PYTHON310_TAG) $(HUMBLE_PYTHON38_TAG) $(ALPINE_PYTHON38_TAG) $(PYTHON38_SLIM_TAG)
...
build-all: build-noetic build-focal build-focal-python310 build-humble build-humble-python38 build-jammy build-jammy-python38 build-alpine-python38 build-python38-slim
build-noetic:
...
build-python38-slim:
	docker build $(BUILD_OPTIONS) -t $(PYTHON38_SLIM_TAG) -f docker/python38-slim/Dockerfile .
...
run-python38-slim: build-python38-slim
	@echo "Starting interactive python38-slim container..."
	docker run --rm -it --user movai $(PYTHON38_SLIM_TAG) bash
...
test-all: test-noetic test-focal test-focal-python310 test-humble test-humble-python38 test-jammy test-jammy-python38 test-alpine-python38 test-python38-slim
...
test-python38-slim: build-python38-slim
	@echo "Testing python38-slim image with container-structure-test..."
	@$(CONTAINER_STRUCTURE_TEST) test --image $(PYTHON38_SLIM_TAG) --config tests/test-python38-slim.yaml
